...
export class ClinicalTrialsServer {
    ...
    async handleSearchByLocation(args) {
        const params = {
            'format': 'json',
            'pageSize': args?.pageSize || 10
        };
        ...
        let locationQuery = '';
        if (args?.country)
            locationQuery += args.country;
        if (args?.state)
            locationQuery += (locationQuery ? ', ' : '') + args.state;
        if (args?.city)
            locationQuery += (locationQuery ? ', ' : '') + args.city;
    ...
    async handleGetTrialStatistics(args) {
    ...
    calculateStatistics(studies, groupBy) {
        if (!groupBy) {
            return {
    ...
    async handleSearchBySponsor(args) {
        if (!args?.sponsor) {
    ...
    async handleSearchByIntervention(args) {
        if (!args?.intervention) {
    ...
    async handleGetRecruitingStudies(args) {
        const params = {
            'format': 'json',
            'pageSize': args?.pageSize || 10,
    ...
    async handleSearchByDateRange(args) {
        const params = {
            'format': 'json',
            'pageSize': args?.pageSize || 10
        };
    ...
    async handleGetStudiesWithResults(args) {
        const params = {
            'format': 'json',
            'pageSize': args?.pageSize || 10,
    ...
    async handleSearchRareDiseases(args) {
        if (!args?.rareDisease) {
    ...
    async handleGetSimilarStudies(args) {
        if (!args?.nctId || !/^NCT\d{8}$/.test(args.nctId)) {
            throw new McpError(ErrorCode.InvalidParams, 'Valid NCT ID is required (format: NCT########)');
        }
        try {
            // First get the reference study to extract similarity criteria
            const referenceResponse = await this.axiosInstance.get('/studies', {
                params: {
                    format: 'json',
                    'query.term': args.nctId,
                    pageSize: 1
            ...
            const response = await this.axiosInstance.get('/studies', { params: searchParams });
            const studies = response.data.studies || [];
    ...
    async handleSearchByPrimaryOutcome(args) {
        if (!args?.outcome) {
    ...
    async handleSearchInternationalStudies(args) {
        const params = {
            'format': 'json',
            'pageSize': args?.pageSize || 10
        };
